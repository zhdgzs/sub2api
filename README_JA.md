...
- **OpenAI 週間クォータ予測** - Plus、Team、Pro の OAuth アカウントについて期間ごとのローカル使用金額、Token 数、呼び出し回数を記録し、使用率が 5% を超えると最新予測を表示します。アカウント管理の履歴グラフでは期間の開始・終了時刻と使用量の詳細を確認できます。API 使用量の保存後、アカウント単位で 30 秒に制限して更新し、使用率が 2 ポイントを超えて低下した場合に新しい期間へ切り替えます（Free、5h、Spark 固有クォータは対象外）
