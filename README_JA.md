...
- OpenAI OAuth アカウントの編集画面で実験的指紋収束（klno）を有効にします。既定は無効です。設定は `extra.codex_experimental_fingerprint_convergence: true` として保存され、無効にするとキーを削除します。シャドウアカウントは認証情報を持つ親アカウントの実験設定を使用します。
- 有効時は親子スレッドとターンを関連付け、派生時に UUIDv7 の形式とタイムスタンプを保持します。デバイスのみ（`codex_fingerprint_mode: device`）と併用すると、エンドポイントに応じたヘッダー処理と compact のキャッシュキー処理も適用します。
- Codex SSE、WS、Messages、Chat Completions ブリッジの送信ヘッダーは常に `session-id` に統一し、`session_id` / `conversation_id` ヘッダーを削除します。JSON では `session_id` を使います。セッションとスレッドは本文、埋め込み metadata、受信ヘッダー、ヘッダー metadata の順で選び、隔離と投影の前に統一します。独立したキャッシュキーの意味は維持し、キャッシュ親和性の値しかない場合はヘッダー名だけを変更します。
- WS の最初のフレームで接続のセッションとスレッドを固定します。後続フレームで省略した ID は引き継ぎ、変更した場合は接続を閉じて再接続を要求します。`session.update` にも適用します。ターンとウィンドウはフレームごとに更新できます。
- 上流アカウント ID と永続的な指紋シードがない OAuth 認証情報では、アクセストークンの不可逆ハッシュで ID を隔離します。この場合、トークン更新後にセッションとキャッシュの関連付けを再構築します。
- 本フォークでは Codex の ID、compact 探測セッション、長いツール呼び出し ID の派生プレフィックスを `sub2api:` から `zhdgzscust:` に変更しています。設定やデータベース内のシードのリセットは不要です。デプロイ後に対象 ID が変わるため、既存のセッション、キャッシュ、ツール呼び出しの関連付けを再構築する場合があります。
- セッション・スレッド・キャッシュキーの一貫した派生、複合 ID の構造保持、フレームごとのウィンドウ修正は常に有効です。更新や設定切り替えで既存 ID が変わり、キャッシュやセッションの関連付けが再構築される場合があります。実験設定を無効にしても修正前の上流動作には戻りません。
- プロトコルの整合性を目的とし、クォータや過負荷、アカウントのリスク判定の改善を保証しません。データベースのスキーマ変更は不要です。
...
- **OpenAI 週間クォータ予測** - Plus、Team、Pro の OAuth アカウントについて期間ごとのローカル使用金額、Token 数、呼び出し回数を記録し、使用率が 5% を超えると最新予測を表示します。アカウント管理およびユーザー側のサブスクリプションアカウント画面では、期間の開始・終了時刻、使用率、使用量の詳細を履歴グラフで確認できます。ユーザーが参照できるのは、有効なサブスクリプショングループ内のアカウントのみです。API 使用量の保存後、アカウント単位で 30 秒に制限して更新し、使用率が 2 ポイントを超えて低下した場合に新しい期間へ切り替えます（Free、5h、Spark 固有クォータは対象外）
