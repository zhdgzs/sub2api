...
- 有効時はセッションヘッダーを補完し、親子スレッドとターンを関連付け、派生時に UUIDv7 の形式とタイムスタンプを保持します。デバイスのみ（`codex_fingerprint_mode: device`）と併用すると、エンドポイントに応じたヘッダー処理と compact のキャッシュキー処理も適用します。
