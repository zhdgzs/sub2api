...
- **OpenAI 周额度预测** - Plus、Team、Pro OAuth 账号按周期记录本地使用金额、Token 量和调用次数；使用比例超过 5% 后展示最新预测额度，并可在账号管理中查看带起止时间与用量详情的周期历史柱状图。API 用量入库后按账号 30 秒节流更新，比例回落超过 2 个百分点时切换周期（不支持 Free、5h 与 Spark 独立额度）
