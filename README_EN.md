...
- The switch correlates parent/child threads and turns and preserves UUIDv7 version and timestamps during derivation. Combine it with **Device only** (`codex_fingerprint_mode: device`) for endpoint-specific device/header handling and compact cache keys.
- Codex SSE, WS, Messages, and Chat Completions bridges always use the outbound `session-id` header, removing `session_id` / `conversation_id` headers; JSON still uses `session_id`. Session and thread identity is selected from body fields, embedded metadata, inbound headers, then header metadata, in that order, before isolation and projection. Headers and metadata share the result. Independent cache keys retain their semantics; cache-only affinity values only get a canonical header name.
- The first WS frame fixes the connection's session and thread. Later frames inherit omitted identity fields; changing them closes the connection with a reconnect message, including through `session.update`. Turns and windows can still change per frame.
- OAuth credentials without an upstream account ID or persistent fingerprint seed use an irreversible access-token hash as the isolation fallback. Token refresh resets session and cache affinity for these accounts.
- This fork uses `zhdgzscust:` instead of `sub2api:` as the derivation prefix for Codex identities, compact probe sessions, and long tool call IDs. No configuration or database seed reset is needed. Deployment changes the affected IDs, so existing session, cache, and tool call associations may need to be re-established.
...
- **OpenAI Weekly Quota Prediction** - Tracks local usage cost, tokens, and request counts by period for Plus, Team, and Pro OAuth accounts. Predictions appear after usage exceeds 5%, with start/end times, usage ratio, and usage details available in the account-management and user subscription-account history charts. Users can only read accounts in their active subscription groups. Updates are throttled to 30 seconds per account after API usage is stored; a drop of more than 2 percentage points starts a new period (Free, 5h, and Spark-specific quotas are excluded)
