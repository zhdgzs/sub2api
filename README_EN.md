...
- **OpenAI Weekly Quota Prediction** - Tracks local usage cost, tokens, and request counts by period for Plus, Team, and Pro OAuth accounts. Predictions appear after usage exceeds 5%, with start/end times, usage ratio, and usage details available in the account-management history chart. Updates are throttled to 30 seconds per account after API usage is stored; a drop of more than 2 percentage points starts a new period (Free, 5h, and Spark-specific quotas are excluded)
