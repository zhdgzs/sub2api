...
func (h *AccountHandler) listAccountsFiltered(ctx context.Context, platform, accountType, planType, status, search string, groupID int64, privacyMode, sortBy, sortOrder string) ([]service.Account, error) {
	page := 1
	pageSize := dataPageCap
	var out []service.Account
	for {
		items, total, err := h.adminService.ListAccounts(ctx, page, pageSize, platform, accountType, planType, status, search, groupID, privacyMode, sortBy, sortOrder)
		if err != nil {
			return nil, err
		}
		out = append(out, items...)
...
func (h *AccountHandler) resolveExportAccounts(ctx context.Context, ids []int64, c *gin.Context) ([]service.Account, error) {
	...
	planType, err := normalizeOpenAIPlanTypeFilter(c.Query("plan_type"))
	if err != nil {
		return nil, err
	}
	...
	return h.listAccountsFiltered(ctx, platform, accountType, planType, status, search, groupID, privacyMode, sortBy, sortOrder)
}
