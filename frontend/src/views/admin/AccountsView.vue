        ...
        >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                v-if="isOpenAIQuotaHistoryAccount(row)"
                ...
                @click="handleViewQuotaHistory(row)"
              >
...
const quotaHistoryAcc = ref<Account | null>(null)
...
function isOpenAIQuotaHistoryAccount(row: Account): boolean {
  if (row.platform !== 'openai' || row.type !== 'oauth' || row.parent_account_id != null) return false
  const plan = getAccountPlanType(row)?.trim().toLowerCase()
  return plan === 'plus' || plan === 'team' || plan === 'pro'
}
