export default {
  ...
  admin: {
    ...
    riskControl: {
      ...
      keywordBlockingMode: '关键词/API 策略',
      ...
      localRules: {
        enabled: '启用本地 Prompt 规则',
        enabledHint: '迁移 codex2api 正则评分规则；默认只记录，所有副作用独立控制。',
        action: '命中后动作',
        actionRecord: '只记录',
        actionRecordDesc: '写入风控日志，按配置继续或跳过 OpenAI Moderations API。',
        actionBlock: '拦截请求',
        actionBlockDesc: '在前置拦截模式下命中即拒绝请求；观察模式下仍只记录。',
        scanScope: '扫描范围',
        scanLatestUserInput: '最新用户输入',
        scanLatestUserInputDesc: '复用当前风控中心输入提取逻辑，默认更保守。',
        scanFullTextContext: '完整文本上下文',
        scanFullTextContextDesc: '扫描 instructions、messages、prompt、input 等文本，跳过 base64、URL 和文件字段。',
        threshold: '普通阈值',
        strictThreshold: '严格阈值',
        maxTextLength: '最大扫描长度',
        skipApiAfterHit: '命中后跳过 API',
        skipApiAfterHitHint: '开启后本地规则命中且未拦截时不再调用 OpenAI Moderations API。',
        countForAutoBan: '计入自动封禁',
        countForAutoBanHint: '开启后本地规则命中会进入现有封禁累计窗口。',
        recordHash: '写入风险哈希',
        recordHashHint: '开启后本地规则命中会写入 Redis 风险哈希集合。',
        emailOnHit: '发送命中邮件',
        emailOnHitHint: '开启后本地规则命中复用内容审计邮件通知。',
        builtinRules: '内置规则',
        builtinRulesHint: 'codex2api 内置 cyber abuse 规则；关闭单条规则可立即降低误报。',
        builtinRuleCount: '{count} 条规则',
        ruleName: '规则名',
        category: '分类',
        weight: '权重',
        enabledColumn: '启用',
        pattern: '正则',
        customRules: '自定义规则',
        customRulesHint: '用于补充内置规则；保存时后端会校验正则。',
        addCustomRule: '添加规则',
        customRulesEmpty: '暂无自定义规则',
        testTitle: '规则试跑',
        testHint: '只测试本地规则，不写日志、不调用 OpenAI Moderations API、不触发封禁或邮件。',
        testButton: '测试规则',
        testing: '测试中',
        testPlaceholder: '输入一段 Prompt 试跑本地规则',
        testHit: '命中本地规则',
        testPass: '未命中',
        ...
        keywordOnlyNotice: '当前为「仅关键词」策略：OpenAI Moderations API 不在路径中，本地规则不会重新打开 API 调用。',
        apiOnlyNotice: '当前为「仅 API」策略：关键词列表不生效；本地 Prompt 规则由这里的开关独立控制。',
      },
      overview: {
        status: '运行状态',
        enabled: '已启用',
    ...
    accounts: {
      ...
      dataExportAccount: '导出此账号',
      dataExportIncludeProxies: '导出代理（导出账号关联的代理）',
      dataImport: '导入',
      copyAccessToken: '复制 access_token',
      accessTokenCopied: 'access_token 已复制',
      accessTokenNotFound: '未找到 access_token',
      copyAccessTokenFailed: '复制 access_token 失败',
      moreActions: '更多操作',
      dataActions: '数据操作',
      toolActions: '工具',
      viewColumns: '列显示',
      ...
      priorityInvalid: '请输入大于等于 1 的整数优先级',
      ...
      selectTestPrompt: '选择测试提示词',
      testModel: '测试模型',
      testPrompt: '提示词："{prompt}"',
      testPromptIdentity: '模型身份',
      testPromptHi: 'hi',
      testPromptBubbleSort: 'Python 冒泡排序',
      testPromptCustom: '自定义',
      customTestPromptLabel: '自定义提示词',
      customTestPromptPlaceholder: '输入本次连接测试要发送的提示词',
      customTestPromptRequired: '请输入自定义提示词',
